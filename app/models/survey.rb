...
class Survey < ActiveRecord::Base # rubocop:todo Metrics/ClassLength
  ...
  def totals_avg(totals)
    return if totals.empty? || totals.sum.zero?

    (totals.sum.to_f / totals.size).round 1
  end

  def percent_change(totals, current)
    return if totals.empty? || !current || current.zero?

    avg = totals_avg totals
    return if !avg || avg.zero?
  ...
  def prior_years
    all_surveys.map(&:to_s).reject { |year| year >= to_s }
  end
  ...
  def survey_year_collection
    all_surveys.each_with_object({}) do |survey, collection|
